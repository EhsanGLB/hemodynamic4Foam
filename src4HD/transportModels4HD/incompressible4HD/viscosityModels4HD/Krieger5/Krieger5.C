...
Foam::viscosityModels::Krieger5::calcNu() const
{
    ...
    return nuPlasma_ * pow((1-H/Hcrit_),-1*((a_+b_*exp(-1*c_*H)+beta_*pow((1+pow(lambda_*strainRate(),2)),(-1*nuK_)))));
}
...
:
    ...
    nuPlasma_(Krieger5Coeffs_.lookup("nuPlasma")),
...
)
{
    ...
    Krieger5Coeffs_.lookup("nuPlasma") >> nuPlasma_;
